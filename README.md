...
| Knowledge / workflow connector | `loopx connect`<br>`loopx lark-meeting-prep`<br>`loopx lark-kanban`<br>`loopx value-connectors` | LoopX state projected into docs, boards, GitHub, or domain workflows while LoopX remains the source of truth. | Existing work surfaces become agent-aware without copying private state into public artifacts. |
...
| Meeting preparation | Reads bounded, user-authorized meeting sources into an owner-private packet and emits a sink-safe todo projection. | `loopx lark-meeting-prep`, [Lark meeting preparation](docs/lark-meeting-preparation.md) |
